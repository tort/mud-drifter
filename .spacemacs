...
It should only modify the values of Spacemacs settings."
  ...
  (setq-default
   ...
   evil-escape-key-sequence "C-["
   ))

(setq org-agenda-text-search-extra-files
      '("~/org/capture-notes.org"
        "~/org/notes.org"
        "~/org/archive-tasks.org"
        ))

(setq org-capture-templates
      '(("t" "Todo" entry (file+headline "~/org/capture-tasks.org" "Tasks"))
        ("n" "Notes" item (file+headline "~/org/capture-notes.org" "Notes") "%? %U\n%i\n%a")
        ))

(setq org-refile-targets
      '((org-agenda-files :maxlevel . 3)
        ("~/org/notes.org" :maxlevel . 3)
        ("~/org/archive-tasks.org" :maxlevel . 3)
        ))
...
before packages are loaded."
  (define-key evil-normal-state-map (kbd "SPC i [") 'evil-surround-region)
...
(with-eval-after-load 'company
  ...
  (define-key company-active-map (kbd "C-u") 'evil-delete-whole-line)
  )
